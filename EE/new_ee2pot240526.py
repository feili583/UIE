...
def ace2pot(sentences,ner,relations,events,sentence_start):
    ...
    for id in range(len(sentences)):
        ...
        tag.append(relation_tag + event_tag)
        # tag.append(event_tag)
        # index.append(ner_index+relation_index + event_index)
        index.append(relation_index + event_index)
        # index.append(event_index)
    ...
    ent2id = dict()
    ind = 0
    for ner in list(set(ner_tags)):
        ent2id[ner] = ind
        ind += 1
    for relation in list(set(relation_tags)):
        ent2id[relation] = ind
        ind += 1
    for event in list(set(event_tags)):
        ent2id[event] = ind
        ind += 1
    for role in list(set(role_tags)):
        ent2id[role] = ind
        ind += 1


    with open('./ACE/ent2id.json','w',encoding='utf-8') as f:
        f.write(json.dumps(ent2id,ensure_ascii=False,indent=4))
...
def pot2file_v0(file,sen,tag,index,sentence_start):
...
def pot2file(file,sen,tag,index,sentence_start):
    '''"label": {"organization": {"星海": [[0, 1]], "星海赛事": [[31, 34]]}}
    "label": {"game": {"魔兽争霸3": [[17, 21]]}, "position": {"选手": [[32, 33]]}}'''
    with open(file,'w',encoding='utf-8') as f:
        json_dict = dict()
        for id in range(len(sen)):
            json_dict['text'] = sen[id]
            json_dict['label'] = dict()
            tmp=[]
            all_tmp=[]
            # print(tag[id])
            # print(index[id])
            for idx in range(len(tag[id])):
                ...
                if len(index[id][idx])>2:
                    ...
                    if index[id][idx][0] > index[id][idx][-2]:
                        if 'r_' + tag[id][idx] not in json_dict['label'].keys():
                            json_dict['label']['r_' + tag[id][idx]] = dict()
                        start_index = min(index[id][idx][0]-sentence_start[id],index[id][idx][-2]-sentence_start[id])
                        end_index = max(index[id][idx][1]-sentence_start[id],index[id][idx][-1]-sentence_start[id])
                        json_dict['label']['r_' + tag[id][idx]][' '.join(text[start_index : end_index + 1])] = [[start_index, end_index]]
                        
                    else:
                        if tag[id][idx] not in json_dict['label'].keys():
                            json_dict['label'][tag[id][idx]] = dict()
                        start_index = min(index[id][idx][0]-sentence_start[id],index[id][idx][-2]-sentence_start[id])
                        end_index = max(index[id][idx][1]-sentence_start[id],index[id][idx][-1]-sentence_start[id])
                        json_dict['label'][tag[id][idx]][' '.join(text[start_index : end_index + 1])] = [[start_index, end_index]]
                else:
                    if tag[id][idx] not in json_dict['label'].keys():
                        json_dict['label'][tag[id][idx]] = dict()
                        # print(text[0:1])
                    json_dict['label'][tag[id][idx]][' '.join(text[index[id][idx][0] - sentence_start[id] : index[id][idx][1] - sentence_start[id] + 1])] = [[index[id][idx][0] - sentence_start[id], index[id][idx][1] - sentence_start[id]]]
...
def ace():
    ...
    pot2file('./ACE/pot_train_0529.json',sen,tag,index,sentence_start)
    print(len(sentences)) 
    ...
    # sen,tag,index,sentence_start=ace2pot(sentences,ner,relations,events,sentence_start)
    # pot2file('./ACE/pot_dev_0529.json',sen,tag,index,sentence_start)
    # print(len(sentences))
    ...
    # sen,tag,index,sentence_start=ace2pot(sentences,ner,relations,events,sentence_start)
    # pot2file('./ACE/pot_test_0529.json',sen,tag,index,sentence_start)
...
def ace_event():
    ...
    pot2file('./ACE_event/pot_train_1027.txt',sen,tag,index,sentence_start)

    ace_data=get_data('./ACE/dev.json')
    doc_key,sentences,ner,relations,events,sentence_start=get_ace(ace_data)
    dic_to_file('./ACE/doc_dev.json',doc_key)
    sen,tag,index,sentence_start=ace2pot(sentences,ner,temp_list,events,sentence_start)
    pot2file('./ACE_event/pot_dev_1027.txt',sen,tag,index,sentence_start)

    ace_data=get_data('./ACE/test.json')
    doc_key,sentences,ner,relations,events,sentence_start=get_ace(ace_data)
    dic_to_file('./ACE/doc_test.json',doc_key)
    sen,tag,index,sentence_start=ace2pot(sentences,ner,temp_list,events,sentence_start)
    pot2file('./ACE_event/pot_test_1027.txt',sen,tag,index,sentence_start)
...
def ace_add_pot(data):
    ...
    with open('./ACE_add/valid_pattern.json','w',encoding='utf-8') as f:
...
def ace_add_event():
    ...
    train_data,test_data,dev_data=get_ace_add('./ACE_add/english.oneie.json',train_doc,test_doc,dev_doc)
    train_sentences,train_tags, train_all_tags=ace_add_pot(train_data)
    ace_add_file('./ACE_add_event/pot_train_1027.txt',train_sentences,train_tags, train_all_tags)

    test_sentences,test_tags, test_all_tags=ace_add_pot(test_data)
    ace_add_file('./ACE_add_event/pot_test_1027.txt',test_sentences,test_tags, test_all_tags)

    dev_sentences,dev_tags, dev_all_tags=ace_add_pot(dev_data)
    ace_add_file('./ACE_add_event/pot_dev_1027.txt',dev_sentences,dev_tags, dev_all_tags)
