...
24.08.01将关系只使用单向，并使用68个标签，减少了关系标签，使用一级关系标签'''
...
def ace2pot(sentences,ner,relations,events,sentence_start):
    ...
    for id in range(len(sentences)):
        ...
        for relation in relations[id]:
            # print(relation)
            relation_tag_tmp = relation[-1].split('.')[0]
            relation_tag.append(relation_tag_tmp)
            relation_index.append(relation[0:-1])
            relation_tags.append(relation_tag_tmp)
            if relation_tag_tmp not in tags_dict['relation_entity'].keys():
                tags_dict['relation_entity'][relation_tag_tmp]=[]
            tags_dict['relation_entity'][relation_tag_tmp].append(ner_dict[str(relation[0])+','+str(relation[1])])
            tags_dict['relation_entity'][relation_tag_tmp].append(ner_dict[str(relation[2])+','+str(relation[3])])
            tags_dict['relation_entity'][relation_tag_tmp]=list(set(tags_dict['relation_entity'][relation_tag_tmp]))
...
def ace():
    ...
    pot2file('./ACE/pot_train_240801.txt',sen,tag,index,sentence_start)
    print(len(sentences))
    ...
    # sen,tag,index,sentence_start=ace2pot(sentences,ner,relations,events,sentence_start)
    # pot2file('./ACE/pot_dev_240801.txt',sen,tag,index,sentence_start)
    # print(len(sentences))
    ...
    # sen,tag,index,sentence_start=ace2pot(sentences,ner,relations,events,sentence_start)
    # pot2file('./ACE/pot_test_240801.txt',sen,tag,index,sentence_start)
...
def ace_add_pot(data):
    ...
    for data_ in data:
        ...
        for relation in data_['relation_mentions']:
            ...
            relation_subtype_tmp = relation['relation_subtype'].split(':')[0]
            relations.append([min(argument1,argument2),max(argument1_end,argument2_end),relation_subtype_tmp])
            for id_start in range(argument1,argument1_end+1):
                for id_end in range(argument2,argument2_end+1): 
                    all_relaitons.append([min(id_start,id_end),max(id_start,id_end),relation_subtype_tmp])
            relation_tags.append(relation_subtype_tmp)
            if relation_subtype_tmp not in tags_dict['relation_entity'].keys():
                tags_dict['relation_entity'][relation_subtype_tmp]=[]
            tags_dict['relation_entity'][relation_subtype_tmp].append(ner_types[relation['arguments'][0]['entity_id']])
            tags_dict['relation_entity'][relation_subtype_tmp]=list(set(tags_dict['relation_entity'][relation_subtype_tmp]))
...
def ace_add():
    ...
    ace_add_file('./ACE_add/pot_train_240801.txt',train_sentences,train_tags, train_all_tags)

    test_sentences,test_tags, test_all_tags=ace_add_pot(test_data)
    ace_add_file('./ACE_add/pot_test_240801.txt',test_sentences,test_tags, test_all_tags)

    dev_sentences,dev_tags, dev_all_tags=ace_add_pot(dev_data)
    ace_add_file('./ACE_add/pot_dev_240801.txt',dev_sentences,dev_tags, dev_all_tags)
