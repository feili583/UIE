...
23.10.31只留下关系,只留下事件
23.12.24加入反向关系
24.09.27第一行将实体加入所有的可能的边，事件和关系只加入头尾'''
        ...
        for id in range(len(sen)):
            ...
            for idx in range(len(tag[id])):
                ...
                if len(index[id][idx])>2:
                    ...
                    for ss in range(index[id][idx][0] - sentence_start[id], index[id][idx][1] - sentence_start[id] + 1):
                        for tt in range(ss, index[id][idx][1] - sentence_start[id] + 1):
                            all_tmp.append(str(ss)+','+str(tt + 1)+' '+tag[id][idx])
...
def ace():
    ...
    pot2file('./ACE/pot_train_0927.txt',sen,tag,index,sentence_start)
    print(len(sentences)) 
    ...
    sen,tag,index,sentence_start=ace2pot(sentences,ner,relations,events,sentence_start)
    pot2file('./ACE/pot_dev_0927.txt',sen,tag,index,sentence_start)
    print(len(sentences))
    ...
    sen,tag,index,sentence_start=ace2pot(sentences,ner,relations,events,sentence_start)
    pot2file('./ACE/pot_test_0927.txt',sen,tag,index,sentence_start)
